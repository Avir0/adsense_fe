...
function InfluencerDashboard() {
  ...
  const [botInput, setBotInput] = useState("");
  const [botMessages, setBotMessages] = useState([
    { from: "bot", text: "Hi! I'm your Adchain AI. Ask about captions, hashtags, growth, content ideas." },
  ]);
  ...
  const submitProof = async (id) => {
    ...
    toast({ title: "Proof submitted", status: "success" });
    setProofInputs((prev) => ({ ...prev, [id]: { link: "", description: "" } }));
    fetchAccepted();
  };
  ...
  const handleBotSend = () => {
    ...
    const msg = botInput.toLowerCase();
    let reply = "Use strong hooks, CTA, and storytelling.";
    if (msg.includes("caption")) reply = "Try: 'This changed my workflow 🔥 #ad #productivity'";
    if (msg.includes("hashtags")) reply = "#viral #creator #reels #growth #adchain";
    if (msg.includes("engagement")) reply = "Post 6-9pm, use reels, ask questions.";

    setBotMessages((prev) => [
      ...prev,
      { from: "user", text: botInput },
      { from: "bot", text: reply },
    ]);
    setBotInput("");
  };

  const earnings = acceptedAds.filter((a) => a.proof?.submittedAt).length * 500;

  // ---------------- UI ----------------
  return (
    <AppLayout>
      <Box>
        <Heading mb={1}>Influencer Dashboard</Heading>
        <Text color="gray.500" mb={6}>Welcome, {user?.name}</Text>
        ...
        <Tabs isFitted variant="soft-rounded" colorScheme="orange">
          ...
          <TabPanels>
              ...
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                {feedAds.map((ad) => (
                  <Box key={ad._id} bg="white" p={5} borderRadius="xl">
                    <Heading size="sm">{ad.title}</Heading>
                    <Text fontSize="sm" mt={2}>{ad.description}</Text>
            ...
            {/* PROCESSING */}
            <TabPanel>
              {acceptedAds.filter(a => !a.proof?.submittedAt).map((ad) => (
                <Box key={ad._id} bg="white" p={5} borderRadius="xl" mb={4}>
                  <Heading size="sm">{ad.title}</Heading>
                  ...
                  <Textarea
                    mt={2}
                    placeholder="Description"
                    value={proofInputs[ad._id]?.description || ""}
                    onChange={(e) =>
                      setProofInputs((prev) => ({
                        ...prev,
                        [ad._id]: { ...prev[ad._id], description: e.target.value },
                      }))
                    }
                  />

                  <Button mt={2} size="sm" onClick={() => submitProof(ad._id)}>
                    Submit Proof
                  </Button>
                </Box>
              ))}
            ...
            <TabPanel>
              {acceptedAds.filter(a => a.proof?.submittedAt).map((ad) => (
                <Box key={ad._id} bg="white" p={5} borderRadius="xl" mb={3}>
                  <Heading size="sm">{ad.title}</Heading>
                  <Badge colorScheme="green">Completed</Badge>
                  <Text fontSize="sm">🔗 {ad.proof?.link}</Text>
                </Box>
              ))}
            </TabPanel>
            ...
            <TabPanel>
              <Box bg="white" p={6} borderRadius="xl">
                ...
                <Text fontSize="3xl" fontWeight="bold">₹ {earnings}</Text>
              </Box>
            </TabPanel>
                ...
                <VStack align="stretch" spacing={3} maxH="300px" overflowY="auto">
                  {botMessages.map((m, i) => (
                    <Box
                      ...
                      bg={m.from === "user" ? "orange.100" : "gray.100"}
                      p={3}
                      borderRadius="lg"
                    >
                      {m.text}
